...
final class ReactorHooksHelper {
	 ...
	 * {@code RunStyle.SYNC} and there is no {@link RecordingPropagator} between input
	...
	public static boolean shouldDecorate(Publisher<?> p) {
		...
		while (true) {
			...
			if (isRecordingPropagator(current)) {
				return false;
			}
	...
	public static boolean isRecordingPropagator(Publisher<?> current) {
		return current instanceof RecordingPropagator || isScopePassingLifter(getLifterName(current));
	}
