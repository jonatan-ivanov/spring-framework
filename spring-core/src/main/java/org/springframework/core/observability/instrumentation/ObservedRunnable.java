...
public class ObservedRunnable implements Runnable {
	...
	public ObservedRunnable(Recorder<?> recorder, IntervalEvent intervalEvent, Runnable delegate) {
		this.recorder = recorder;
		this.recording = this.recorder.recordingFor(intervalEvent);
		this.delegate = delegate;
	}

	public ObservedRunnable(Recorder<?> recorder, Runnable delegate) {
		this(recorder, () -> "async", delegate);
	}

	@Override
	public void run() {
		IntervalRecording<?> started = this.recording.start();
		try {
			delegate.run();
		} catch (Exception ex) {
			started.error(ex);
