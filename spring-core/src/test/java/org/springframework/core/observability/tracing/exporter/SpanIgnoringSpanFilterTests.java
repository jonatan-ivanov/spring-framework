...
class SpanIgnoringSpanFilterTests {
	...
	void should_not_handle_span_when_present_in_main_list_of_spans_to_skip() {
		...
		assertThat(handler.isExportable(namedSpan())).isFalse();
	}
	...
	void should_use_cached_entry_for_same_patterns() {
		...
		assertThat(SpanIgnoringSpanFilter.cache).containsKey("someOtherName");

		export(handler("a"));
		export(handler("b"));
		export(handler("c"));

		assertThat(SpanIgnoringSpanFilter.cache).containsKey("someOtherName").containsKey("a").containsKey("b")
				.containsKey("c");
	}
