...
class DocumentedSpanAssertionsTests {
	...
	void should_fail_when_assertion_is_on_and_a_key_is_unknown() {
		assertThatThrownBy(() -> assertThatKeyIsValid("unknown_key", SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("The key [unknown_key] is invalid");
		assertThatThrownBy(() -> assertThatKeyIsValid(A_FOO_TAG, SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("The key [foo.key] is invalid");
	}

	@Test
	void should_fail_when_assertion_is_on_and_an_event_is_unknown() {
		assertThatThrownBy(() -> assertThatEventIsValid("unknown_event", SPAN_WITH_PREFIX))
				.hasMessageContaining("The event [unknown_event] is invalid");
		assertThatThrownBy(() -> assertThatEventIsValid(A_BAR_EVENT, SPAN_WITH_PREFIX))
	...
	void should_fail_when_assertion_is_on_and_a_key_is_known_but_wrongly_prefixed() {
		assertThatThrownBy(() -> assertThatKeyIsValid("bar.key", SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("Also it has start with [foo.] prefix");
		assertThatThrownBy(() -> assertThatKeyIsValid(MyTagsWithNotMatchingPrefix.A_BAR_TAG, SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("Also it has start with [foo.] prefix");
	}

	@Test
	void should_fail_when_assertion_is_on_and_an_event_is_known_but_wrongly_prefixed() {
		assertThatThrownBy(() -> assertThatEventIsValid("bar.value", SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("Also it has start with [foo.] prefix");
		assertThatThrownBy(() -> assertThatEventIsValid(A_BAR_EVENT, SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("Also it has start with [foo.] prefix");
	}

	@Test
	void should_fail_when_assertion_is_on_and_a_key_is_known_but_dynamic_key_is_not_matched() {
		assertThatThrownBy(() -> assertThatKeyIsValid("notmatching", SPAN_WITH_DYNAMIC_ENTRIES))
				.hasMessageContaining("The key [notmatching] is invalid. You can use only one matching [%s.key]");
		assertThatThrownBy(() -> assertThatKeyIsValid(MySimpleTag.A_SIMPLE_TAG, SPAN_WITH_DYNAMIC_ENTRIES))
	...
	void should_fail_when_assertion_is_on_and_an_event_is_known_but_dynamic_value_is_not_matched() {
		assertThatThrownBy(() -> assertThatEventIsValid("notmatching", SPAN_WITH_DYNAMIC_ENTRIES))
				.hasMessageContaining("The event [notmatching] is invalid. You can use only one matching [%s.value]");
		assertThatThrownBy(() -> assertThatEventIsValid(MySimpleEvent.A_SIMPLE_EVENT, SPAN_WITH_DYNAMIC_ENTRIES))
	...
	void should_fail_when_assertion_is_on_and_name_is_invalid() {
		assertThatThrownBy(() -> assertThatNameIsValid("unknown_name", SPAN_WITH_NOT_MATCHING_PREFIX))
				.hasMessageContaining("The name [unknown_name] is invalid");
	}

	@Test
	void should_fail_when_assertion_is_on_and_name_is_not_matching() {
		assertThatThrownBy(() -> assertThatNameIsValid("unknown_name", SPAN_WITH_DYNAMIC_ENTRIES)).hasMessageContaining(
	...
	void should_fail_when_span_was_ended_but_not_started() {
		assertThatThrownBy(() -> assertThatSpanStartedBeforeEnd(
				new ImmutableAssertingSpan(SPAN_WITH_NOT_MATCHING_PREFIX, BDDMockito.mock(Span.class))))
