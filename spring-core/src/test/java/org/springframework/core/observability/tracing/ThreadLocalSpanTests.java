...
class ThreadLocalSpanTests {
	...
	void should_set_and_retrieve_values_from_thread_local() {
		...
		assertThat(spanAndScope).isNotNull();
		assertThat(spanAndScope.getSpan()).isSameAs(firstSpan);
		...
		assertThat(spanAndScope).isNotNull();
		assertThat(spanAndScope.getSpan()).isSameAs(secondSpan);
		...
		span.remove();
		spanAndScope = span.get();
		assertThat(spanAndScope).isNull();
	}
	...
	void should_create_a_new_thread_local_span() {
		...
		// then
		assertThat(span.get().getSpan()).isSameAs(firstSpan);
	}
