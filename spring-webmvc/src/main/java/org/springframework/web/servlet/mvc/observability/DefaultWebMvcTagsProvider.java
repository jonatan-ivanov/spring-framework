...
public class DefaultWebMvcTagsProvider implements WebMvcTagsProvider {
	...
	@Override
	public Iterable<Tag> getTags(HttpServerRequest request, HttpServerResponse response, Object handler,
			Throwable exception) {
		List<Tag> tags = new ArrayList<>(Arrays.asList(WebMvcTags.method(request), WebMvcTags.uri(request, response, this.ignoreTrailingSlash), WebMvcTags.exception(exception), WebMvcTags.status(response)));
		for (WebMvcTagsContributor contributor : this.contributors) {
			contributor.getTags(request, response, handler, exception).forEach(tags::add);
		}
		return tags;
	}

	@Override
	public Iterable<Tag> getLongRequestTags(HttpServerRequest request, Object handler) {
